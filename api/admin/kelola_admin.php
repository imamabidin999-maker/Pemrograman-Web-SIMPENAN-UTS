...
$current_user = strtolower($_COOKIE['username'] ?? '');
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body class="flex min-h-screen bg-[#f8fafc]">
    ...
    <main class="flex-1 p-12 flex flex-col items-start">
        <h1 class="text-5xl font-black uppercase italic tracking-tighter text-kementan mb-2">Manajemen Admin</h1>
        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-10">Daftar Tim Pengelola Sistem</p>

        <?php if ($current_user === 'imam'): ?>
        <div class="bg-white neo-gov-card p-8 w-full max-w-5xl mb-10">
        ...
        <?php else: ?>
        <div class="bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-10 rounded-r-xl w-full max-w-5xl">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="font-bold text-xs">
                    ...
                    while($row = mysqli_fetch_assoc($res)): 
                        $row_user = strtolower($row['username']);
                    ?>
                    <tr class="border-b-2 border-gray-50 hover:bg-emerald-50 transition-colors">
                        ...
                        <td class="p-5 italic uppercase text-kementan font-black">
                            ...
                            <?php if($row_user === 'imam'): ?>
                                <span class="ml-2 text-[8px] bg-yellow-400 text-[#1e3d1a] px-2 py-0.5 rounded font-black uppercase">Master</span>
                            <?php endif; ?>
                        </td>
                        <td class="p-5 text-yellow-500 uppercase tracking-tighter">Full Access</td>
                        <td class="p-5 text-center">
                            <?php if($row_user === $current_user): ?>
                                <span class="px-3 py-1 rounded-lg text-[8px] font-black uppercase bg-gray-100 text-gray-400">
                                    Sesi Aktif
                                </span>
                            <?php elseif ($current_user === 'imam' && $row_user !== 'imam'): ?>
